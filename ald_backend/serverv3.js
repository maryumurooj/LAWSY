  ...
  try {
    const judgment = await Judgment.findByPk(judgmentId, {
      include: [
        {
          model: Citation,
          attributes: [ 'citationCourtName', 'publicationYearId'],
          include: [
            {
              model: PublicationYear,
              attributes: ['publicationYearNo']
            }
          ]
        },
        {
  ...
  try {
    ...
    rows.forEach((row) => {
      ...
      if (row.shortNoteId) {
        ...
        if (row.longNoteText && !existingShortNote.longNotes.some((note) => note.longNoteText === row.longNoteText)) {
          existingShortNote.longNotes.push({
            ...
            longNoteParaJudgmentNo: row.longNoteParaJudgmentNo
          });
        }
      }
    });
